...
def test_qodercn_skills_project_into_config_home_and_are_idempotent(tmp_path: Path) -> None:
    ...
    active = materialize_qodercn_skills(home, profile=ProviderProfileSpec(inherit_skills=True))
    repeated = materialize_qodercn_skills(home, profile=ProviderProfileSpec(inherit_skills=True))
    ...
    disabled = materialize_qodercn_skills(home, profile=ProviderProfileSpec(inherit_skills=False))
...
def test_qodercn_skills_preserve_unmarked_user_owned_conflicts(tmp_path: Path) -> None:
    home = tmp_path / 'managed-home'
    ...
    assert payload['label'] == 'qodercn-ccb-skill:ccb-clear'
